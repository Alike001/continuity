...
The guarded submission service in `scripts/orchestrator.mjs` is the first permissioned orchestration boundary. It accepts a signed FCC result, requires a bearer operator token, checks the referenced ciphertext through the snapshot-store API, persists one durable job per action ID, and invokes `cast send` only when both execution flags and the executor key are present. Dry-run is the default. A failed RPC or reverted receipt remains a failed job for inspection, and submitted jobs are reconciled from receipts after restart. Stronger operator authentication than a localhost bearer token remains required before internet exposure.
